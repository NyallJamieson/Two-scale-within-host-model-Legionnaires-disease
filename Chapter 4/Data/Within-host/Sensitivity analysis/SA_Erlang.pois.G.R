...
for (i in 1:1000){
  ...
  MODEL.onestep <- function(x,params){
    ...
    transitions <- list(
      ...
      rupture <- c(rpois(n=1,lambda = mean),0,0,-1)
    )
  ...
  MODEL.simul <- function(y,maxstep=100000000,threshold=50661){
    ...
    t <- output[k,1]
    if (x["L"]!=0){
      return(c("G"=mean,x["time"]))
    }
  }
  
  results <- dplyr::bind_rows(results,as.data.frame(na.omit(do.call(rbind,parallel::mclapply(1:1000,MODEL.simul,mc.cores=150)),na.action="omit")))
}
