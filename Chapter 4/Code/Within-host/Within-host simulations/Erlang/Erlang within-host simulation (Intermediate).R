...
MODEL.onestep <- function(x,params){
  ...
  if (event==5){
    ...
    size <- round(sample_G(sample_size=1))
    transitions <- list(
...
for (i in 1:500){
  print(i)
  xstart <- c(time=0,L=i,M1=0,M2=0,M3=0)
  results <- dplyr::bind_rows(results,as.data.frame(na.omit(do.call(rbind,parallel::mclapply(1:1000,MODEL.simul,mc.cores=25)),na.action="omit")))
}
