...
simulate_nMtau_R <- function(y){
  ...
  for (i in t) {
    ...
    if (M > 0) {
      ...
      L <- L + num_rupture * rpois(n=1, lambda=59.58144)
    } else {
...
for (j in 299:500){
  print(j)
  xstart <- c("L"=j)
  results <- dplyr::bind_rows(results,as.data.frame(na.omit(do.call(rbind,parallel::mclapply(1:1000,simulate_nMtau_R,mc.cores=25)),na.action="omit")))
}
