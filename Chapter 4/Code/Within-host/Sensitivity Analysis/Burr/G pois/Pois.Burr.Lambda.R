...
for (i in 1:1000){
  ...
  sampler <- function(n) {
  u <- runif(n)
  return(n / sum(sapply(u, function(ui) {
    uniroot(function(x) pdburr(x, a = a_val, b = b_val, T = T_val) - ui, 
            lower = 0, upper = 1000)$root
            })))
    }
  
  lambda <- sampler(100000)
  ...
  simulate_nMtau_R <- function(y){
    ...
    for (i in t) {
      ...
      if (M > 0) {
        ...
        F_t <- pdburr(ages, a = a_val, b = b_val, T = T_val)  # CDF at time t
        F_t_plus_dt <- pdburr(dt + ages, a = a_val, b = b_val, T = T_val)  # CDF at t + dt
        ...
        L <- L + sum(rpois(n = num_rupture, lambda = 59.58144))
      } else {
    ...
    if (ill == 1){
      output <- c(lambda,time)
      names(output) <- c("lambda","time")
      return(output)
    }
    
    if (cleared ==1){
      return(c("lambda" = NA, "time" = NA))
    }
  }
...
write.csv(results, "SA_Burr.Pois.lambda.csv")
