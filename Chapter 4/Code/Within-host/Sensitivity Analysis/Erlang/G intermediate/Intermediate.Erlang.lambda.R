...
for (i in 1:1000){
  ...
  simulate_nMtau_R <- function(y){
    ...
    for (i in t) {
      ...
      if (M > 0) {
        ...
        F_t <- pgamma(ages, shape = 3, rate = lambda)  # CDF at time t
        F_t_plus_dt <- pgamma(dt + ages, shape = 3, rate = lambda)  # CDF at t + dt
  ...
  results <- dplyr::bind_rows(results,as.data.frame(na.omit(do.call(rbind,parallel::mclapply(1:10,simulate_nMtau_R,mc.cores=64)),na.action="omit")))
}

write.csv(results, "SA_Erlang.Intermediate.lambda.csv")