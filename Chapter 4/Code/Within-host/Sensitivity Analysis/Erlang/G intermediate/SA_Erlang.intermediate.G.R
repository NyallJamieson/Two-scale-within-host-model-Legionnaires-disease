...
for (i in 1:4){
  ...
  results <- dplyr::bind_rows(results,as.data.frame(na.omit(do.call(rbind,parallel::mclapply(1:4,simulate_nMtau_R,mc.cores=1)),na.action="omit")))
}

write.csv(results, "SA_Erlang_Intermediate_.csv")